    ...
    classMethods: {
      ...
      parseNoteInfo: function (body) {
        ...
        return {
          ...
          marks: Note.extractNoteMarks(parsed.meta, $)
        }
      },
      ...
      extractNoteMarks: function (meta, $) {
        var marks = []
        var rawmarks = []
        if (meta.marks && (typeof meta.marks === 'string' || typeof meta.marks === 'number')) {
          var metamarks = ('' + meta.marks).split(',')
          for (let i = 0; i < metamarks.length; i++) {
            var text = metamarks[i].trim()
            if (text) rawmarks.push(text)
          }
        } else {
          var h6s = $('h6')
          h6s.each(function (key, value) {
            if (/^marks/gmi.test($(value).text())) {
              var codes = $(value).find('code')
              for (let i = 0; i < codes.length; i++) {
                var text = S($(codes[i]).text().trim()).stripmarks().s
                if (text) rawmarks.push(text)
              }
            }
          })
        ...
        for (let i = 0; i < rawmarks.length; i++) {
          ...
          for (let j = 0; j < marks.length; j++) {
            if (marks[j] === rawmarks[i]) {
              found = true
              break
            }
          }
          if (!found) { marks.push(rawmarks[i]) }
        }
        return marks
      },
