...
var History = {
  ...
  historyGetSomeone: historyGetSomeone,
  historyGetEverybody: historyGetEverybody
}
...
function updateHistory (userid, noteId, document, time) {
  if (userid && noteId && typeof document !== 'undefined') {
    getHistory(userid, function (err, history) {
      ...
      noteHistory.marks = noteInfo.marks
...
function historyGetSomeone (req, res) {
  if (req.isAuthenticated()) {
    getHistory(req.params.userId, function (err, history) {
      if (err) return response.errorInternalError(res)
      if (!history) return response.errorNotFound(res)
      res.send({
        history: parseHistoryToArray(history)
...
function historyGetEverybody (req, res) {
  models.User.findAll().then(users => {
    let ids = {};

    res.send({
      history: users.map(user => JSON.parse(user.history))
                    .reduce((histories, history) => histories.concat(history), [])
